...
func TestViewRestGetObjectSuccess(t *testing.T) {
	testList := []struct {
		inObject  interface{}
		outCode   int
		outObject interface{}
	}{
		{inObject: nil, outCode: http.StatusNoContent},
		{inObject: "Message", outCode: http.StatusOK, outObject: "Message"},
	}

	for testId, testCase := range testList {
		router, err := rest.MakeRouter(rest.Get("/api", func(w rest.ResponseWriter, r *rest.Request) {
			view := NewViewRestGetObject()
...
func TestViewRestGetObjectError(t *testing.T) {
	testList := []struct {
		inError   error
		inCode    int
		outCode   int
