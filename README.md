...
GLOBAL OPTIONS:
   ...
   --aws-id 				        AWS access id key with S3 read/write permissions for specified bucket (required if not ec2). [$AWS_ACCESS_KEY_ID]
   --aws_secret 			        AWS secret key with S3 read/write permissions for specified bucket (required if not ec2). [$AWS_SECRET_ACCESS_KEY]
