...
func TestMigration_1_2_8(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping TestMigration_1_2_8 in short mode.")
	}
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			err := m.Apply(ctx, migrate.MakeVersion(1, 2, 8), migrations)
			assert.NoError(t, err)

			deploymentsIndicesNames := []string{
				IndexDeploymentsActiveCreated,
			}
			...
			for _, indexName := range deploymentsIndicesNames {
