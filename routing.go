...
func NewRouter(c config.ConfigReader) (rest.App, error) {
	...
	routes := []*rest.Route{
		...
		rest.Get("/api/0.0.1/deployments/:id", mvc.NewGetObjectController(deploymentModel, mvc.NewViewRestGetObject())),

		// Devices
		rest.Get("/api/0.0.1/devices/:id/update", mvc.NewGetObjectController(models.NewDeviceUpdateModel(dbSession, fileStorage), mvc.NewViewRestGetObject())),
	}
